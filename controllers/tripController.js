...
exports.addTrip = async (req, res) => {
    const { cityName, tripName, guestName, time, days, price, avatar, description } = req.body; // Thêm description
    try {
        // Lưu trip mới vào database
        const newTrip = new Trip({ cityName, tripName, guestName, time, days, price, avatar, description }); // Thêm description
        await newTrip.save();
...
exports.updateTrip = async (req, res) => {
    ...
    try {
        const trip = await Trip.findByIdAndUpdate(
            id,
            { cityName, tripName, guestName, time, days, price, avatar, description }, // Thêm description
            { new: true }
        );
        if (!trip) {
